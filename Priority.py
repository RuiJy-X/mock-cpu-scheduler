...
class Priority(CPUScheduler):
    def __init__(self):
        self.process = {} # {"process1": [5,2,priority]} index 0 = burst time, index 1 = arrival time
    ...
    def __str__(self):
        return "Priority"
    def input_process(self,burst_time, arrival_time,priority):
        """_summary_
        ...
        """
        self.process[new_process()] = [burst_time,arrival_time,priority] #create new process (p0,p1,p2..pn) with : [burst_time,arrival_time]
        self.display_process = self.process.copy()
        
    def display(self):
        print(f"Process\tBurst Time\tArrival Time\tPriority")
        for processes,time in self.display_process.items():
            print(f"{processes}\t{time[0]}\t\t{time[1]}\t\t{time[2]}")
    ...
    def insert_ready(self,current_time):
        ...
        sorted_arrivals = sorted(self.process.items(),key=lambda item: (item[1][2],item[1][1], item[0]))
    ...
    def schedule(self):
        ...
        while self.ready or self.has_remaining_burst():
            ...
            process_priority = process_time[2]
            
            
            time_slice = process_burst_time #if burst time is lower, it chooses that, else take time quantum
            ...
            self.process[process]= [burst_left,process_arrival_time,process_priority]
            ...
            self.gantt.append({"process_name": process,"start":start,"end":end,"duration":time_slice,"arrival_time":process_arrival_time,"priority":process_priority})
    ...
    def get_waiting_times(self):
        ...
        self.finished_process_summary = process_summary[["process_name","Total_Burst_Time","Arrival_Time","Waiting_Time","Priority"]].set_index("process_name")
    ...
    def print_gantt_chart(self):
        ...
        ax_table.set_title(f'Priority Scheduling Summary', pad=7, fontweight='bold')

        table = ax_table.table(
            cellText=summary_df.values, 
            colLabels=["Total Burst Time (ms)", "Arrival Time (ms)", "Waiting time (ms)","Priority (ms)"], 
        ...
        ax.set_title('Priority Schedule Gantt Chart')
